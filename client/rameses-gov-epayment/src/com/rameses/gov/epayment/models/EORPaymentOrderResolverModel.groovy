...
public class EORPaymentOrderResolverModel {
    ...
    private void processItem( po ) {
        logHandler.writeln("\n[ "+ po.controlno +" ] verify payment..."); 
        def pay = resolverSvc.verifyPayment( po ); 
        if ( pay?.objid ) {
            logHandler.writeln("   Status: OK"); 
            logHandler.writeln("[ "+ po.controlno +" ] post payment..."); 
            resolverSvc.postPayment( pay ); 
        }
        else {
            logHandler.writeln("   Status: "+ pay?.msg); 
        }
    }
}